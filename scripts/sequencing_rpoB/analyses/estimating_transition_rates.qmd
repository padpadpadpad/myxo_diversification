...
ancestral_states_ape3 <- readRDS(here('data/sequencing_rpoB/processed/transition_rates/ancestral_states_ape3.rds'))
```
...
custom_matrix2[custom_matrix2 %in% c(25)] <- 0
...
So this parameter is super important in the model. So our best model is **ancestral_states_ape3**.
...
The transition rates from both approaches do not match! This is not good news. I wonder if we could set some symmetric rates to get an overall better fit and more consistent convergence.

```{r set_some_symmetric_rates}
# input the rates into the custom matrix
custom_matrix3 <- custom_matrix

# make some transitions symmetric:
# marine mud to freshwater and vice versa
custom_matrix3[custom_matrix3 %in% c(21)] <- 4
custom_matrix3[custom_matrix3 > 21] <- custom_matrix3[custom_matrix3 > 21] -1

ancestral_states_ape5 <- ace(hab_pref, tree, model=custom_matrix3, type="discrete")

anova(ancestral_states_ape3, ancestral_states_ape5)
...
```
So this model fits better, and now lots of transitions that were positive are now zero, which would hint that we can remove them. Lets plot the transition matrix to see.
...
ace_transition_df(ancestral_states_ape5) %>%
  ...
  labs(y = 'state 1',
  x = 'state 2',
  title = paste('all rates different with', length(unique(custom_matrix3[custom_matrix3 != 0])), 'free parameters', sep = ' ')) +
  coord_fixed() +
  scale_color_manual(values = c('black', 'red'))
```
Some pretty strong rates have now become 0 (which feels a bit weird to me).

Instead we can try and set a bunch of low rates (<0.1) to zero and see how we from there.

```{r}
custom_matrix4 <- custom_matrix

# get index of rates which are <0.1
which(ancestral_states_ape3$rates < 0.1)

custom_matrix4[custom_matrix4 %in% which(ancestral_states_ape3$rates < 0.1)] <- 0
...
anova(ancestral_states_ape6, ancestral_states_ape3)
...
# check AIC of each model
mod_compare <- tibble(mod = c('ER', 'SYM', 'ARD', 'ancestral_states_ape3', 'ancestral_states_ape4', 'ancestral_states_ape5', 'ancestral_states_ape6', 'ancestral_states_ape_7'),
...
ace_transition_df(ancestral_states_ape7) %>%
  ...
  labs(y = 'state 1',
  x = 'state 2',
  title = paste('all rates different with', length(unique(custom_matrix5[custom_matrix5 != 0])), 'free parameters', sep = ' ')) +
  coord_fixed() +
  scale_color_manual(values = c('black', 'red'))
