...
We can look at whether any given state is a net sink or net source using both the total number of transitions and the instantaneous transition rates.
...
We have a tip estimate of the diversification rate from our BAMM analysis that we have binned into either being "high" or "slow" diversification. We want to run a discrete character evolution model to look at whether transitions across environments result in a shift into the "high" diversification rate state. However, the model would be too big to combine our diversification rate bin and our seven habitat preferences.

Instead we can look for a logical split in the dataset. To do this we can use the mass action expectation that the number of transitions between two habitat preferences should be equivalent to their prevalence in our tip states. We will do this for all available transitions and see which ones are over- or under-represented.
