...
table_lambda <- idparslist$lambdas %>%
  ...
  add_header_row(values = c('Speciation rate:'), colwidths = 15) %>%
...
table_mu <- idparslist$mus %>%
  ...
  flextable() %>%
  # add column name above
  add_header_row(values = c('Extinction rate:'), colwidths = 15) %>%
  align(align = 'left', part = 'header', i=1) %>%
  font(fontname = 'Times', part = 'all') %>%
  fontsize(size = 16, part = 'all') %>%
  fix_border_issues() %>%
...
to_bold <- c(5,6,7,10,11,12,13,14,15,16,17)
length(to_bold)
...
table_transition <- idparslist$Q %>%
  ...
  flextable() %>%
  # add column name above
  add_header_row(values = c('Transition rates:'), colwidths = 16) %>%
  align(align = 'left', part = 'header', i=1) %>%
  font(fontname = 'Times', part = 'all') %>%
  fontsize(size = 16, part = 'all') %>%
  ...
  hline(i = c(5, 10), border = fp_border_default()) %>%
  vline(j = c(1, 6, 11), border = fp_border_default()) %>%
  fix_border_issues() %>%
  autofit() %>%
  hline_top(border = fp_border_default(width = 0), 
            part = "header")
...
# save this out
ggsave('plots/manuscript_plots/ctd3_model_params.png', height = 8, width = 10)
