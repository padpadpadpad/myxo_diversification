...
d_habpref <- read.csv(here('data/sequencing_rpoB/phyloseq/myxococcus/habitat_preference/summary/habitat_preference_asv_check.csv')) %>%
  mutate(habitat_preference2 = ifelse(habitat_preference %in% c('freshwater:marine_mud:terrestrial', 'freshwater:marine_mud', 'marine_mud:terrestrial'), 'marine_mud_generalist', habitat_preference),
...
inits_ard <- rep(3, length(argnames(lik_ard)))
inits_sym <- rep(3, length(argnames(lik_sym)))
inits_er <- rep(3, length(argnames(lik_er)))
inits_trans <- rep(3, length(argnames(lik_transient)))

# run first three models
mod_sym <- find.mle(lik_sym, inits_sym, method = 'optim', control = list(maxit = 50000))
...
# do AIC comparison
AIC(mod_sym, mod_ard, mod_er, mod_custom1, mod_custom2, mod_custom3, mod_custom4, mod_custom5, mod_custom6, mod_custom7, mod_custom8, mod_trans) %>% arrange(AIC)
...
diversitree_df <- get_diversitree_df(mod_custom8, coding$hab_pref_num, coding$hab_pref)
