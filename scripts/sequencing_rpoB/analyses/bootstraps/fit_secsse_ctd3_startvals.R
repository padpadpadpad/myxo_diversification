...
for(j in 1:length(trees)){
  ...
  # save this out
  if(transitions == 'all'){saveRDS(inits_ml, paste('data/sequencing_rpoB/processed/secsse/init_vals_ml/bootstraps/', name, '_asvboot', num_tree, 'alltransitions.rds', sep = ''))}
  if(transitions == 'hidden'){saveRDS(inits_ml, paste('data/sequencing_rpoB/processed/secsse/init_vals_ml/bootstraps/', name, '_asvboot', num_tree, '.rds', sep = ''))}
