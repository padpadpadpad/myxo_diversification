...
We constrained our tree based on the phylogeny in XX, so we will just colour by the families that were used in the constraint.

```{r find_common_taxa}
# constrained families
constrained_families <- d_labels$part3 %>% unique() %>% .[!is.na(.)]
...
The top 9 families represent 89% of all the tips. This is quite good so lets go with this for now. If we wanted we could plot all of these and see if there is an obvious cut-off where its not worth adding more colours.
...
d_meta <- mutate(d_meta, family_common = ifelse(family2 %in% c(constrained_families, 'uncertain'), family2, 'unconstrained'),
                 family_common = ifelse(family2 %in% c(d_common$family2, 'uncertain', 'unconstrained'), family2, 'rare constrained'))
...
cols <- c(colorRampPalette(brewer.pal(11, "Spectral"))(nrow(d_common)), 'black', 'grey', 'dark grey')
names(cols) <- c(d_common$family2, 'uncertain', 'unconstrained', 'rare constrained')
...
names(cols_hab) <- c('terrestrial', 'freshwater', 'mud_and_shore', 'generalist')
...
Ok this looks pretty darn amazing. There are a couple of things to take home from this. Firstly, the phylogenetic data aligns pretty well with the tree we created from the rpoB sequencing. It is by no means perfect (e.g. Haliangiaceae and Polyangiaceae are kind of everywhere), but given this is a \~250bp region of a single gene it links pretty well to the taxonomy as created from GTDB which uses over a hundred single-copy-marker genes to make its taxonomy of the whole of bacteria.

Second, we can see clusters of colours at the tips of the phylogenetic tree showing that habitat preferences are not randomly distributed. It is difficult to see but when an ASV can live in two habitats its outline and fill colours are different. We have found phylogenetic signal in the data previously by calculating Pagel's lambda and the D-statistic for binary traits.
...
-   Your habitat preferences are defined as all the habitats you at least as much as is expected by their availability.
