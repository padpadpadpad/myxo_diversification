...
table_lambda <- idparslist$lambdas %>%
  ...
  add_header_row(values = c('Speciation rate:'), colwidths = 20) %>%
...
table_mu <- idparslist$mus %>%
  ...
  flextable() %>%
  # add column name above
  add_header_row(values = c('Extinction rate:'), colwidths = 20) %>%
  align(align = 'left', part = 'header', i=1) %>%
  font(fontname = 'Times', part = 'all') %>%
  fontsize(size = 12, part = 'all') %>%
  fix_border_issues() %>%
...
to_bold <- c(6,7,8,12,13,14,15,16,17,18,19)
length(to_bold)
...
table_transition <- idparslist$Q %>%
  ...
  flextable() %>%
  # add column name above
  add_header_row(values = c('Transition rates:'), colwidths = 21) %>%
  align(align = 'left', part = 'header', i=1) %>%
  font(fontname = 'Times', part = 'all') %>%
  fontsize(size = 12, part = 'all') %>%
  ...
  bold(~ c(`1C`) %in% to_bold, ~ `1C`) %>%
  bold(~ c(`2C`) %in% to_bold, ~ `2C`) %>%
  bold(~ c(`3C`) %in% to_bold, ~ `3C`) %>%
  bold(~ c(`4C`) %in% to_bold, ~ `4C`) %>%
  bold(~ c(`5C`) %in% to_bold, ~ `5C`) %>%
  bold(~ c(`1D`) %in% to_bold, ~ `1D`) %>%
  bold(~ c(`2D`) %in% to_bold, ~ `2D`) %>%
  bold(~ c(`3D`) %in% to_bold, ~ `3D`) %>%
  bold(~ c(`4D`) %in% to_bold, ~ `4D`) %>%
  bold(~ c(`5D`) %in% to_bold, ~ `5D`) %>%
  # make column name from blank
  set_header_labels(from = '') %>%
  hline(i = c(5,10,15), border = fp_border_default()) %>%
  vline(j = c(1, 6,11,16), border = fp_border_default()) %>%
  fix_border_issues() %>%
  autofit() %>%
  hline_top(border = fp_border_default(width = 0), 
            part = "header")
...
# save this out
ggsave('plots/manuscript_plots/ctd4_model_params.png', height = 10, width = 10.5)
