...
```{r plot_tree, fig.height=10, fig.width=5}
...
We can now plot a circular tree with the habitat preferences imposed on the tree.

```{r plot_tree2, fig.height=9, fig.width=10}
# first only plot taxonomy, also make non circular so we can see groupings properly
