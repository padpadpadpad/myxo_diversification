...
librarian::shelf(here, tidyverse, ggtree, ggnewscale, RColorBrewer, patchwork, ape, phytools, MetBrewer, flextable, officer)
```
...
# write function to add colour circle inside flextable
# from here: https://stackoverflow.com/questions/71068389/add-coloured-dot-in-new-column-using-dplyr
colour_choice = function(x){
  if (x == 'freshwater specialist') {
    "#16317d"
  } else if (x =='freshwater + land generalist') {
    "#ffcd12"
  } else if (x == 'marine specialist'){
    "#a40000"
  }
    else if (x == 'marine generalist'){
    "#721b3e"
  }
    else('#007e2f')
  }
...
table <- flextable(d_table) %>%
  set_header_labels(first_col = '',
                    habitat_preference2 = 'Habitat preference',
...
tree_plot3 <- tree_plot %<+% d_meta_new +
  ...
  scale_color_manual('Habitat preference (tip points)', values = cols_hab2, labels = c('freshwater + land generalist', 'freshwater specialist', 'marine generalist', 'marine specialist', 'land specialist')) +
  scale_size_manual(values = c(0.6, 3)) +
  guides(color = guide_legend(override.aes = list(size = 5)),
         size = 'none')
...
tree_plot4 <- tree_plot3 + inset_element(backbone_tree, -0.12, 0.0, 0.12,0.19)

table <- png::readPNG(here('plots/manuscript_plots/hab_pref_table.png'), native = TRUE)

(tree_plot4) / (p2 + table) + plot_layout(ncol = 1, heights = c(0.8, 0.2), tag_level = 'new')

# save out
ggsave(here('plots/manuscript_plots/Figure_2.png'), last_plot(), height = 9.5, width = 11)
