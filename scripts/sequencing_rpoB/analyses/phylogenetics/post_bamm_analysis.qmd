...
In general, we want the effective sample size values to be at least 200 (and 200 is on the low side, but might be reasonable for very large datasets). Both are now close to 200 with a 40% burnin.
...
edata <- getEventData(tree, eventdata = here('data/sequencing_rpoB/bamm/bamm_1_event_data.txt'), burnin = 0.4)
...
The model has converged, and we also now have a narrow(ish) range of potential values for the number of range shifts (between 23 and 34).
...
We can see the best model with 27 shifts is \>20 away from the next best model, implying strong evidence that the best fit to the data contains 27 shifts.
...
d_prior <- plotPrior(mcmcout, expectedNumberOfShifts=500, burnin = 0.4) %>%
  data.frame() %>%
  janitor::clean_names() %>%
...
However, as can be seen from the summary, even the single best shift configuration has a really low posterior probability (0.00125). Although the best model supports 27 shifts, not all of these are core shifts. There are only 18-22 core shifts identified in the best model shift configurations here.
...
This picks a model with 22 rate shifts over the whole tree.
