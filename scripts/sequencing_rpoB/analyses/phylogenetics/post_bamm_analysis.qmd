...
From the online documentation of BAMM, we are following [Section 8](http://bamm-project.org/postprocess.html#bammtools): Analysing BAMM Output with BAMMtools.
...
#mbt2 <- getMeanBranchLengthTree(msc_config, rate = "speciation")
...
scale_color_gradientn('Net diversification (branch colours)', colors = met.brewer(name='Hiroshige', direction=-1, override.order = F), breaks=c(min(d_tree_bamm$log_edge_length, na.rm = TRUE) + abs(min(d_tree_bamm$edge_length, na.rm = TRUE))*0.2, max(d_tree_bamm$log_edge_length, na.rm = TRUE) * 0.95), labels=c("Slow","Fast")) +
...
p2 <- p1 %<+% d_meta +
  ...
  scale_color_manual('Habitat preference (tip points)', values = cols_hab, labels = c('freshwater + land generalist', 'freshwater specialist', 'marine generalist', 'marine specialist', 'land specialist')) +
...
ggsave(here('plots/sequencing_rpoB/analyses/bamm_tree.png'), p3, height = 9, width = 12)
```
...
p_tiprates <- ggplot(tip_rates, aes(forcats::fct_reorder(hab_pref_axis, n), net_diversification)) +
  ...
  scale_fill_manual('Habitat preference', values = cols_hab, labels = c('freshwater + land generalist', 'freshwater specialist', 'marine generalist', 'marine specialist', 'land specialist')) +
  ylim(c(0, 10))
...
We can now assemble a figure of these results for the manuscript.
...
ggsave(here('plots/sequencing_rpoB/analyses/bamm_tree.png'), tree_plot2, height = 8, width = 8)

tree_plot2 <- image_read(here('plots/sequencing_rpoB/analyses/bamm_tree.png'), density = 300)
tree_plot2 <- image_trim(tree_plot2)

# make plot
image_ggplot(tree_plot2) + legend + plot_rateshifts + p_rtt + p_tiprates + plot_layout(design = layout, heights = c(0.75, 0.25))

ggsave(here('plots/manuscript_plots/Figure_3.png'), last_plot(), width = 13, height = 11)
...
saveRDS(mod, here('data/sequencing_rpoB/processed/tipbamm_pgls.rds'))
saveRDS(contrasts, here('data/sequencing_rpoB/processed/tipbamm_pgls_contrasts.rds'))
```
