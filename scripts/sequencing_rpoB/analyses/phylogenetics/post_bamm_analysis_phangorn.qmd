...
# load in tree
tree <- ape::read.tree(here('data/sequencing_rpoB/raxml/trees/myxo_asv/myxo_asv_phangorn.tre'))
...
# create function to scale data between 0 and 1
scale_01 <- function(x){(x-min(x))/(max(x)-min(x))}

# create data frame
d_ltt <-  ape::ltt.plot.coords(tree) %>%
  data.frame() %>%
  mutate(time2 = scale_01(time))
...
mcmcout <- read.csv(here("data/sequencing_rpoB/bamm/bamm_asv_phangorn_mcmc_out.txt"), header=TRUE)
...
edata <- getEventData(tree, eventdata = here('data/sequencing_rpoB/bamm/bamm_asv_phangorn_event_data.txt'), burnin = 0.3)
...
The maintainers or BAMM suggest that (usually) the best overall model from a BAMM analysis is the model with the highest Bayes factor relative to the null model, $M_{0}$, which has zero rate shifts. However, we do not have any samples of zero shifts in our postburn in sample, instead the minimum number of shifts is `r min(postburn$N_shifts)`! However, we do have zero shifts in our preburn-in samples as can be seen here: `r min(mcmcout$N_shifts)`.
...
# list file
mcmc_file = here("data/sequencing_rpoB/bamm/bamm_asv_phangorn_mcmc_out.txt")
...
```{r best_model_bayes}
filter(d_bayes_factors, cum_diff < 20) %>%
  pull(n_shifts) %>%
  as.numeric() %>%
  summary()
```
...
This picks a model with `r length(getShiftNodesFromIndex(edata, index = msc_set$sampleindex))` rate shifts over the whole tree.
...
ggsave(here('plots/sequencing_rpoB/analyses/bamm_tree_phangorn.png'), last_plot(), height = 9, width = 12)
```
...
# save plot out
ggsave(here('plots/sequencing_rpoB/analyses/bamm_rate_through_time_phangorn.png'), last_plot(), height = 5, width = 12)
```
...
# save plot out
ggsave(here('plots/sequencing_rpoB/analyses/bamm_tip_rates_phangorn.png'), last_plot(), height = 5, width = 17)
