...
#------------------------------#
# Second for the 97.7% tree ####
#------------------------------#

# load in best raxml tree from treePL CV
tree1 <- read.tree('data/sequencing_rpoB/raxml/trees/myxo_97.7/myxo_97.7_treepl_cv.tre')

# load in bootstrapped tree with tbe values from raxml - unrooted
tree2 <- read.tree('data/sequencing_rpoB/raxml/trees/myxo_97.7/myxo_97.7.raxml_tbe.tree')
...
# save out tree
write.tree(tree2, 'data/sequencing_rpoB/raxml/trees/myxo_97.7/myxo_97.7.raxml_tbe_v2.tree')
...
# load in tree2 now after rooting correctly
tree2 <- readNexus('data/sequencing_rpoB/raxml/trees/myxo_97.7/myxo_97.7.raxml_tbe_rooted.tree', format = 'raxml')
...
# do all the tip names match up between the treePL tree and the bootstrapped tree?
# got to make sure the rotation of the tree is correct!
sum(tree1$tip.label == tree2$tip.label) == length(tree1$tip.label)
# needs to be true to go forwards
...
# make sure is ultrametric using phytools
is.ultrametric(tree1)

tree1 <- force.ultrametric(tree1)

# save out tree and keep node labels
write.tree(tree1, 'data/sequencing_rpoB/raxml/trees/myxo_97.7/myxo_97.7_treepl_cv_node_labels.tre')

#---------------------------#
# Third for the 95% tree ####
#---------------------------#

# load in best raxml tree from treePL CV
tree1 <- read.tree('data/sequencing_rpoB/raxml/trees/myxo_95/myxo_95_treepl_cv.tre')

# load in bootstrapped tree with tbe values from raxml - unrooted
tree2 <- read.tree('data/sequencing_rpoB/raxml/trees/myxo_95/myxo_95.raxml_tbe.tree')
...
# save out tree
write.tree(tree2, 'data/sequencing_rpoB/raxml/trees/myxo_95/myxo_95.raxml_tbe_v2.tree')
...
# load in tree2 now after rooting correctly
tree2 <- readNexus('data/sequencing_rpoB/raxml/trees/myxo_95/myxo_95.raxml_tbe_rooted.tree', format = 'raxml')
...
# save out tree and keep node labels
write.tree(tree1, 'data/sequencing_rpoB/raxml/trees/myxo_95/myxo_95_treepl_cv_node_labels.tre')
