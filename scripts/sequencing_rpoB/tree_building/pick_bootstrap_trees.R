...
# load in trees
trees <- read.tree('data/sequencing_rpoB/raxml/trees/myxo_95/boots.raxml.bootstraps')

# pick 9 random trees
trees <- trees[sample(1:length(trees), 9)]

# load in rooted tree with family name tip labels
tree1 <- read.nexus('data/sequencing_rpoB/raxml/trees/myxo_95/myxo_95.raxml_rooted.tre')
...
for(i in 1:9){
  ...
  # save out tree
  write.tree(trees[[i]], paste0('data/sequencing_rpoB/raxml/trees/myxo_95//raxml_bootstrap_', i, '.tree'))
}
