...
here::i_am('scripts/sequencing_rpoB/tree_building/append_tiplabels_v2.R')
...
for(i in 1:length(percent_similarity)){
  ...
  # define tree file
  in_tree <- here(paste('data/sequencing_rpoB/raxml/trees/myxo_', percent_similarity[i], '/myxo_', percent_similarity[i],'.raxml_tbe.tree', sep = ''))
  
  # define output file
  out_tree <- here(paste('data/sequencing_rpoB/raxml/trees/myxo_', percent_similarity[i], '/myxo_', percent_similarity[i],'.raxml_tbe_family.tre', sep = ''))
